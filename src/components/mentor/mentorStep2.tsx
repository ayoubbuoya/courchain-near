interface MentorStep2Props {
  courseType: string;
  setCourseType: (value: string) => void;
  courseTypeOther: string;
  setCourseTypeOther: (value: string) => void;
}

export default function MentorStep2({
  courseType,
  setCourseType,
  courseTypeOther,
  setCourseTypeOther,
}: MentorStep2Props) {
  return (
    <div className="shadow-custom-green backdrop-blur-[3.13rem] custom-linear-border rounded-2xl my-3 lg:my-8">
      <div className="lg:p-20 py-1 px-2">
        <div>
          <h1
            style={{
              WebkitTextStroke: "0.5px #E2B8E9",
            }}
            className="uppercase text-purple font-capriola font-normal text-xl text-center lg:text-left lg:text-[1.88rem] lg:leading-[1.88rem] py-2  "
          >
            TYPE DE COURS QUE VOUS COMPTEZ Créer
          </h1>
        </div>
        <div className="flex items-center justify-between flex-wrap ">
          <div className="w-full lg:max-w-[55%] 2xl:max-w-[49%] flex justify-center items-center ">
            <div>
              <p className="text-schemes-secondary font-poppins font-normal text-base lg:text-xl">
                {`On est curieux de savoir un peu plus sur vous, voilà pourquoi
                    on vous a préparer ces questions pour vous connaitre
                    d’avantage.`}
              </p>
              <div className="py-1 lg:py-4">
                <div
                  onClick={() => setCourseType("vid+quizz")}
                  className="flex items-center gap-3 justify-start py-3 "
                >
                  <div
                    className={`bg-[#F2F2F4] w-6 h-6 lg:w-7 lg:h-7 rounded-full cursor-pointer `}
                  >
                    {courseType === "vid+quizz" && (
                      <div className="w-2 h-2 bg-black rounded-full mx-auto my-2"></div>
                    )}
                  </div>
                  <label
                    htmlFor=""
                    className="text-black font-poppins text-sm lg:text-[1.06rem] font-normal"
                  >
                    Sessions vidéos enregistré + quiz
                  </label>
                </div>
                <div className="flex items-center gap-3 justify-start py-3 ">
                  <div
                    onClick={() => setCourseType("vid+pratq")}
                    className={`bg-[#F2F2F4] w-6 h-6 lg:w-7 lg:h-7 rounded-full cursor-pointer `}
                  >
                    {courseType === "vid+pratq" && (
                      <div className="w-2 h-2 bg-black rounded-full mx-auto my-2"></div>
                    )}
                  </div>
                  <label
                    htmlFor=""
                    className="text-black font-poppins text-sm lg:text-[1.06rem] font-normal"
                  >
                    Visioconférences + cas pratiques
                  </label>
                </div>
                <div className="flex items-center gap-3 justify-start py-3 ">
                  <div
                    onClick={() => setCourseType("pratq")}
                    className={`bg-[#F2F2F4] w-6 h-6 lg:w-7 lg:h-7 rounded-full cursor-pointer `}
                  >
                    {courseType === "pratq" && (
                      <div className="w-2 h-2 bg-black rounded-full mx-auto my-2"></div>
                    )}
                  </div>
                  <label className="text-black font-poppins text-sm lg:text-[1.06rem] font-normal">
                    Exercices pratiques préparation aux certificats
                  </label>
                </div>

                <div className="flex items-center gap-3 justify-start py-3 ">
                  <div
                    onClick={() => setCourseType("other")}
                    className={`bg-[#F2F2F4] w-6 h-6 lg:w-7 lg:h-7 rounded-full cursor-pointer `}
                  >
                    {courseType === "other" && (
                      <div className="w-2 h-2 bg-black rounded-full mx-auto my-2"></div>
                    )}
                  </div>
                  <label
                    htmlFor=""
                    className="text-black font-poppins text-sm lg:text-[1.06rem] font-normal"
                  >
                    Autres
                  </label>
                </div>
                {courseType === "other" && (
                  <input
                    style={{
                      backgroundColor: "#F2F2F4",
                      color: "#99999B",
                    }}
                    onChange={(e) => setCourseTypeOther(e.target.value)}
                    value={courseTypeOther}
                    placeholder="Exemple"
                    type="text"
                    className="w-[85%] md:w-[66%] my-2 h-12 font-inter font-normal outline-none  text-[1.06rem]  rounded-lg px-4"
                  />
                )}
              </div>
            </div>
          </div>
          <div className="w-[85%] mb-4 mx-auto lg:w-[39%] lg:m-0 ">
            <img
              className="w-full xl:max-h-[20.94rem]"
              src="/article-review.svg"
              alt=""
            />
          </div>
        </div>
      </div>
    </div>
  );
}
